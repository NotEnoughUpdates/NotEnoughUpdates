...
public class SenitherDungeonsWeight extends DungeonsWeight {
	...
	public void getClassWeight(String className) {
		float currentClassXp = Utils.getElementAsFloat(
			Utils.getElement(player, "experience_skill_" + className),
			0
		);
		double currentClassLevel = ProfileViewer.getLevel(
			...
			Utils.getElementAsInt(
				Utils.getElement(Constants.LEVELING, "leveling_caps.catacombs"),
				50
			),
			false
		)
		...
		double base =
			...
			Utils.getElementAsFloat(
				Utils.getElement(
					Constants.WEIGHT,
					"senither.dungeons.classes." + className
				),
				0
			);
		...
		weightStruct.add(
			new WeightStruct(Math.floor(base), Math.pow(remaining / splitter, 0.968))
		);
	}

	@Override
	public void getDungeonWeight() {
		float catacombsSkillXp = Utils.getElementAsFloat(
		...
		double base =
			Math.pow(level, 4.5) *
			Utils.getElementAsFloat(
				Utils.getElement(Constants.WEIGHT, "senither.dungeons.catacombs"),
				0
			);
