...
public class HorseModifier extends EntityViewerModifier {
	...
	public EntityLivingBase applyModifier(EntityLivingBase base, JsonObject info) {
		if (!(base instanceof EntityHorse horse))
			return null;
		if (info.has("kind")) {
			String type = info.get("kind").getAsString().intern();
			switch (type) {
				case "skeleton" -> horse.setHorseType(4);
				case "zombie" -> horse.setHorseType(3);
				case "mule" -> horse.setHorseType(2);
				case "donkey" -> horse.setHorseType(1);
				case "horse" -> horse.setHorseType(0);
				default -> throw new IllegalArgumentException("Unknown horse type: " + type);
			}
		}
		if (info.has("armor")) {
			...
			if (el.isJsonNull()) {
				...
				Item item = switch (el.getAsString().intern()) {
					case "iron" -> Items.iron_horse_armor;
					case "golden" -> Items.golden_horse_armor;
					case "diamond" -> Items.diamond_horse_armor;
					default -> throw new IllegalArgumentException("Unknown horse armor: " + el.getAsString());
				};
