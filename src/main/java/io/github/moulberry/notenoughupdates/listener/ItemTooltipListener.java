...
public class ItemTooltipListener {
    ...
    public void onItemTooltipLow(ItemTooltipEvent event) {
        ...
        for (String line : event.toolTip) {
            if (line.endsWith(EnumChatFormatting.DARK_GRAY + "Reforge Stone") && NotEnoughUpdates.INSTANCE.config.tooltipTweaks.showReforgeStats) {
                ...
                line = line.replace("\u00A7cR\u00A76a\u00A7ei\u00A7an\u00A7bb\u00A79o\u00A7dw\u00A79 Rune", Utils.chromaString("Rainbow Rune", index, false) + EnumChatFormatting.BLUE);
                ...
                for (String op : NotEnoughUpdates.INSTANCE.config.hidden.enchantColours) {
                    ...
                    Pattern pattern;
                    try {
                        pattern = Pattern.compile("(\\u00A79|\\u00A7(9|l)\\u00A7d\\u00A7l)(?<enchantName>" + enchantName + ") " + "(?<level>[0-9]+|(I|II|III|IV|V|VI|VII|VIII|IX|X|XI|XII|XIII|XIV|XV|XVI|XVII|XVIII|XIX|XX))((\\u00A79)?,|( \\u00A78(?:,?[0-9]+)*)?$)");
                    } catch (Exception e) {
                        continue;
                    } //malformed regex
                    ...
                    while (matcher.find() && matchCount < 5) {
                        ...
                        if (matches) {
                            ...
                            if (!colourCode.equals("z")) {
                                ...
                                line = line.replace("\u00A79\u00A7d\u00A7l" + enchantText, "\u00A7" + colourCode + extraMods + enchantText);
                                line = line.replace("\u00A7l\u00A7d\u00A7l" + enchantText, "\u00A7" + colourCode + extraMods + enchantText);
                            } else {
                                int offset = Minecraft.getMinecraft().fontRendererObj.getStringWidth(line.replaceAll("\\u00A79" + enchantText + ".*", ""));
                                line = line.replace("\u00A79" + enchantText, Utils.chromaString(enchantText, offset / 12f + index, false));

                                offset = Minecraft.getMinecraft().fontRendererObj.getStringWidth(line.replaceAll("\\u00A79\\u00A7d\\u00A7l" + enchantText + ".*", ""));
                                line = line.replace("\u00A79\u00A7d\u00A7l" + enchantText, Utils.chromaString(enchantText, offset / 12f + index, true));
                                offset = Minecraft.getMinecraft().fontRendererObj.getStringWidth(line.replaceAll("\\u00A7l\\u00A7d\\u00A7l" + enchantText + ".*", ""));
                                line = line.replace("\u00A7l\u00A7d\u00A7l" + enchantText, Utils.chromaString(enchantText, offset / 12f + index, true));
                            }
                        }
                    }
                }
            ...
            if (NotEnoughUpdates.INSTANCE.config.tooltipTweaks.showPriceInfoAucItem) {
                if (line.contains(EnumChatFormatting.GRAY + "Buy it now: ") || line.contains(EnumChatFormatting.GRAY + "Bidder: ") || line.contains(EnumChatFormatting.GRAY + "Starting bid: ")) {
                ...
                if (Utils.cleanColour(event.toolTip.get(1)).matches(petToolTipRegex)) {
                    ...
                    if (tag.hasKey("ExtraAttributes")) {
                        if (tag.getCompoundTag("ExtraAttributes").hasKey("petInfo")) {
                            JsonObject petInfo = NotEnoughUpdates.INSTANCE.manager.gson.fromJson(tag.getCompoundTag("ExtraAttributes").getString("petInfo"), JsonObject.class);
                    ...
                    if (petlevel != null) {
                        ...
                        if (petlevel.totalXp > petlevel.maxXP) {
                            ...
                            tooltipText.add(EnumChatFormatting.GRAY + "EXP: " + EnumChatFormatting.YELLOW + myFormatter.format(petlevel.levelXp) + EnumChatFormatting.GOLD + "/" + EnumChatFormatting.YELLOW + myFormatter.format(petlevel.currentLevelRequirement) + " EXP");
                        }
                    }
                }
            }
        ...
        if (NotEnoughUpdates.INSTANCE.config.tooltipTweaks.petExtendExp) {
            ...
            if (event.toolTip.size() > 7) {
                if (Utils.cleanColour(event.toolTip.get(1)).matches(petToolTipRegex)) {
                    ...
                    event.toolTip.add(xpLine + 1, EnumChatFormatting.GRAY + "EXP: " + EnumChatFormatting.YELLOW + myFormatter.format(petLevel.levelXp) + EnumChatFormatting.GOLD + "/" + EnumChatFormatting.YELLOW + myFormatter.format(petLevel.currentLevelRequirement));
    ...
    public void onItemTooltip(ItemTooltipEvent event) {
        ...
        if (Keyboard.isKeyDown(Keyboard.KEY_LCONTROL) && NotEnoughUpdates.INSTANCE.config.hidden.dev && event.toolTip.size() > 0 && event.toolTip.get(event.toolTip.size() - 1).startsWith(EnumChatFormatting.DARK_GRAY + "NBT: ")) {
