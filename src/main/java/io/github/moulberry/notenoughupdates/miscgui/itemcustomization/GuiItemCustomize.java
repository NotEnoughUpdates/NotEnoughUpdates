...
public class GuiItemCustomize extends GuiScreen {
	...
	private void drawScreenDefault(int mouseX, int mouseY, float partialTicks) {
		...
		ArrayList<String> animatedSkullHelp = ItemCustomizeManager.getAnimatedSkullHelp(textFieldCustomItem.getText());
		if (animatedSkullHelp != null) {
			Minecraft.getMinecraft().getTextureManager().bindTexture(GuiTextures.help);
			GlStateManager.color(1, 1, 1, 1);
			int skullHelpX = xCenter + textFieldRename.getWidth() / 2;
			Utils.drawTexturedRect(skullHelpX, yTop, 20, 20, GL11.GL_LINEAR);
